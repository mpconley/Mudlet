...
void XMLexport::writeHost(Host* pHost, pugi::xml_node mudletPackage)
{
    ...
    host.append_attribute("defaultOIDCProvider") = pHost->getDefaultOIDCProvider().toUtf8().constData();

    if (!pHost->getOIDCProviders().isEmpty()) {
        host.append_attribute("oidcProviders") = pHost->getOIDCProviders().join(",").toUtf8().constData();
    }
