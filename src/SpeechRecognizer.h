...
public:
    ...
    // same for every engine, and both backends had grown their own copy - one
    // of which had already drifted into moving to Error on a refusal. A
    // backend implements doStartListening() and inherits the rules.
    void startListening()
    {
        if (state() != State::Ready) {
            ...
            // not a fault, and the contract says a refusal may arrive without
            // a state change.
    ...
    // Finalises the pending utterance. Only Listening has one to finalise.
    void stopListening()
    {
    ...
    // Abandons what is in flight without finalising it. Starting counts: a
    // request still waiting on a permission dialog has no audio to abandon,
    // but leaving it pending means answering the dialog later opens the
    // microphone after the caller asked to stop.
    void cancel()
    {
