...
// Private
void TLuaInterpreter::processQueryMediaTable(lua_State* L, TMediaData& mediaData)
{
    const Host& host = getHostFromLua(L);
    QList<TMediaData> const matchingMediaDataList = host.mpMedia->queryMedia(mediaData);
...
// Private
int TLuaInterpreter::queryMusicAsOrderedArguments(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
    const int numArgs = lua_gettop(L);
    ...
    processQueryMediaTable(L, mediaData);
    return 1;
}

// Private
int TLuaInterpreter::queryMusicAsTableArgument(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#queryMusic
int TLuaInterpreter::queryMusic(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
    ...
    if (lua_gettop(L)) {
        if (lua_istable(L, 1)) {
            return queryMusicAsTableArgument(L);
        }

        return queryMusicAsOrderedArguments(L);
    }
...
// Private
int TLuaInterpreter::querySoundsAsOrderedArguments(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
    const int numArgs = lua_gettop(L);
...
// Private
int TLuaInterpreter::querySoundsAsTableArgument(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
...
// Documentation: https://wiki.mudlet.org/w/Manual:Lua_Functions#querySounds
int TLuaInterpreter::querySounds(lua_State* L)
{
    const Host& host = getHostFromLua(L);
    TMediaData mediaData{};
    ...
    if (lua_gettop(L)) {
        if (lua_istable(L, 1)) {
            return querySoundsAsTableArgument(L);
        }

        return querySoundsAsOrderedArguments(L);
    }
